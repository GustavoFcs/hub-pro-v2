  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-[#999999] text-sm font-mono tracking-widest">IMPORTAÇÃO DE PDF COM ANÁLISE POR INTELIGÊNCIA ARTIFICIAL</p>
      </div>
        ...
        {(['upload', 'gabarito', 'reviewing', 'success'] as Stage[]).map((s, i) => {
          ...
          return (
            <React.Fragment key={s}>
              {i > 0 && <div className={cn('h-px flex-1', isDone ? 'bg-accent' : 'bg-[#333]')} />}
              <span className={cn('text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded border',
                ...
                isDone ? 'text-white border-white/20 bg-white/5' : 'text-[#555] border-[#333]'
              )}>{labels[i]}</span>
            </React.Fragment>
          )
        })}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-1 space-y-1">
                <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Título</label>
                <input type="text" value={titulo} onChange={e => setTitulo(e.target.value)}
                  placeholder="Ex: ENEM 2023  Ciências da Natureza"
                  className="w-full bg-black border border-[#333] rounded-md px-3 py-2 text-sm text-white placeholder:text-[#444] focus:outline-none focus:border-accent" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Ano</label>
                <input type="number" value={ano} onChange={e => setAno(e.target.value)} min={1990} max={new Date().getFullYear() + 1}
                  className="w-full bg-black border border-[#333] rounded-md px-3 py-2 text-sm text-white focus:outline-none focus:border-accent" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Instituição</label>
          ...
          <div onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}
            ...
            {pdfFile ? (
              <div className="text-center">
                <p className="text-white font-bold">{pdfFile.name}</p>
                <p className="text-[#666] text-xs mt-1">{(pdfFile.size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
            ) : (
              <div className="text-center">
                <h3 className="text-lg font-bold text-white mb-1">Arraste o PDF aqui</h3>
                <p className="text-[#666] text-sm">ou clique para selecionar o arquivo</p>
              </div>
            )}
            <input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf"
              onChange={e => { const f = e.target.files?.[0]; if (f) setPdfFile(f) }} />
            {!pdfFile && (
              <Button type="button" className="bg-accent text-white hover:bg-[#E55A2B] px-8 py-5 rounded-md font-bold text-xs uppercase tracking-widest">SELECIONAR PDF</Button>
            )}
          </div>
          ...
          {processing && (
            <div className="bg-secondary p-6 rounded-2xl border border-accent/20 space-y-3">
              ...
              <Progress value={processProgress} className="h-1.5 bg-black" />
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-card border border-border">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={`text-[10px] font-mono uppercase tracking-widest transition-colors ${
                cropMode === 'ai' ? 'text-accent' : 'text-[#555]'
              }`}>IA</span>
              <button
                type="button"
                onClick={toggleCropMode}
                className={`relative w-10 h-5 rounded-full transition-colors ${
                  cropMode === 'manual' ? 'bg-accent' : 'bg-white/10'
                }`}
              >
              ...
              <span className={`text-[10px] font-mono uppercase tracking-widest transition-colors ${
                cropMode === 'manual' ? 'text-accent' : 'text-[#555]'
              }`}>Manual</span>
            </div>
          </div>

          <Button type="submit" disabled={processing || !pdfFile}
            className="w-full bg-accent text-white hover:bg-[#E55A2B] py-6 rounded-md font-bold text-sm uppercase tracking-widest gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {stage === 'gabarito' && (
        <div className="space-y-6">
          ...
          >
            ...
            {gabaritoFile ? (
              <div className="text-center">
                <p className="text-white font-bold text-sm">{gabaritoFile.name}</p>
                <p className="text-[#666] text-xs mt-1">{(gabaritoFile.size / 1024 / 1024).toFixed(2)} MB</p>
                <button onClick={() => { setGabaritoFile(null); setGabaritoResult(null) }}
                  className="mt-2 text-xs font-mono text-[#666] hover:text-red-400 transition-colors">Remover</button>
              </div>
            ) : (
              <div className="text-center">
                <p className="text-sm font-bold text-white">Arraste o PDF do gabarito</p>
                <p className="text-xs text-[#666] mt-1">ou clique para selecionar</p>
              </div>
            )}
            <input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf"
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => setStage('reviewing')}
              className="border-[#333] text-[#999] hover:text-white font-bold text-xs uppercase tracking-widest">
              Pular este passo
            </Button>
            <Button
              onClick={gabaritoResult ? () => setStage('reviewing') : handleScanGabarito}
              disabled={(!gabaritoFile && !gabaritoResult) || scanningGab}
              className="flex-1 bg-accent text-white hover:bg-[#E55A2B] py-6 font-bold text-sm uppercase tracking-widest gap-2 disabled:opacity-50">
              {scanningGab
          ...
          <div className="bg-card rounded-2xl border border-border p-5 flex flex-wrap gap-6 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={() => setQuestions(qs => qs.map(q => ({ ...q, selected: true })))}
                className="border-[#333] text-[#999] hover:text-white text-xs uppercase tracking-widest font-bold">Selecionar todas</Button>
              <Button variant="outline" onClick={() => setQuestions(qs => qs.map(q => ({ ...q, selected: false })))}
                className="border-[#333] text-[#999] hover:text-white text-xs uppercase tracking-widest font-bold">Desmarcar todas</Button>
            </div>
          </div>
      ...
      {stage === 'success' && (
        <div className="flex flex-col items-center gap-8 py-16">
          ...
          <div className="text-center space-y-2">
            <h2 className="text-3xl font-bold text-white uppercase tracking-tight">Importação concluída!</h2>
            <p className="text-[#999] text-sm font-mono">{importedCount} questões foram adicionadas ao banco com sucesso.</p>
          </div>
          <div className="flex gap-4">
            <Button variant="outline" onClick={handleReset}
              className="border-[#333] text-[#999] hover:text-white gap-2 font-bold text-xs uppercase tracking-widest px-8 py-5">
              <UploadCloud size={14} /> NOVA PROVA
            </Button>
            <Button onClick={() => window.location.href = '/admin/gerenciar-provas'}
              className="bg-accent text-white hover:bg-[#E55A2B] px-8 py-5 font-bold text-xs uppercase tracking-widest">VER PROVAS</Button>
          </div>
        </div>
      )}
    </div>
    ...
    <div className={cn('rounded-2xl border transition-all duration-200',
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <button type="button" title={q.searchVideo ? 'Buscar vídeo no YouTube' : 'Não buscar vídeo'}
          ...
          className={cn('transition-colors', q.searchVideo ? 'text-red-400' : 'text-[#444]')}>
          <PlayCircle size={16} />
        </button>
        <button type="button" onClick={() => onUpdate({ expanded: !q.expanded })} className="text-[#555] hover:text-white transition-colors">
          {q.expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        </button>
        <button type="button" onClick={onRemove} className="text-[#444] hover:text-red-400 transition-colors">
          <Trash2 size={14} />
        </button>
      </div>
      {q.expanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-[#1a1a1a] pt-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Alternativas</label>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Matéria</label>
              <input value={q.subject} onChange={e => onUpdate({ subject: e.target.value })}
                className="w-full bg-black border border-[#333] rounded-md px-2 py-1.5 text-xs text-white focus:outline-none focus:border-accent" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Subtópico</label>
              <input value={q.subtopic} onChange={e => onUpdate({ subtopic: e.target.value })}
                className="w-full bg-black border border-[#333] rounded-md px-2 py-1.5 text-xs text-white focus:outline-none focus:border-accent" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Dificuldade</label>
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-mono text-[#555] uppercase tracking-widest">Gabarito</label>
